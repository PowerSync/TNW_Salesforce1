            ...
            try {
                ...
                if ($response->any instanceof stdClass) {
                    if ($this->isSObject($response->any)) {
                        $anArray = array();
                        $sobject = new Salesforce_SObject($response->any);
                        $anArray[] = $sobject;
                        $this->sobjects = $anArray;
                    } else {
                        // this is for parent to child relationships
                        $this->queryResult = new Salesforce_QueryResult($response->any);
                    }
                        ...
                        foreach ($response->any as $key => $item) {
                            if ($item instanceof stdClass) {
                                if ($this->isSObject($item)) {
                                    $sobject = new Salesforce_SObject($item);
                                    ...
                                    array_push($this->queryResult, new Salesforce_QueryResult($item));
                                }
                            } else {
