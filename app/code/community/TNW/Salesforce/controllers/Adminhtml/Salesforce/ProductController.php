 ...
 */
class TNW_Salesforce_Adminhtml_Salesforce_ProductController extends Mage_Adminhtml_Controller_Action
{
    /**
     * @return $this
     */
    ...
    {
        $this->_title($this->__('System'))->_title($this->__('Salesforce API'))->_title($this->__('Product Field Mapping'));
        $this->_initLayout()
            ->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_product'));
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $id == 0) {
            ...
            Mage::register('salesforce_product_data', $model);
    ...
    {
        $itemIds = $this->getRequest()->getParam('product');
        if (!is_array($itemIds)) {
