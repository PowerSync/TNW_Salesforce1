...
class TNW_Salesforce_Adminhtml_Salesforce_OrderController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->_title($this->__('System'))->_title($this->__('Salesforce API'))->_title($this->__('Order Field Mapping'));
        $this->_initLayout()
            ->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_order'));
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $id == 0) {
            ...
            Mage::register('salesforce_order_data', $model);
            ...
            $this->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_order_edit'));
    ...
    protected function _validate($data)
    {
        ...
        foreach ($collection as $_item) {
            if ($_item->getMappingId()) {
                Mage::getSingleton('adminhtml/session')->addError('Duplicate mapping already exists!');
                return false;
            }
        }
