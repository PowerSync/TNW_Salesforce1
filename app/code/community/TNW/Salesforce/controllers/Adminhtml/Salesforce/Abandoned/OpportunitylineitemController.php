...
class TNW_Salesforce_Adminhtml_Salesforce_Abandoned_OpportunitylineitemController extends Mage_Adminhtml_Controller_Action
{
    protected function _initLayout()
    {
        ...
        $this->loadLayout()
            ...
            ->_addBreadcrumb(Mage::helper('tnw_salesforce')->__('Shopping Cart Fields Mapping'), Mage::helper('tnw_salesforce')->__('Shopping Cart Field Mapping'));
    ...
    {
        $this->_title($this->__('System'))->_title($this->__('Salesforce API'))->_title($this->__('Shopping Cart Field Mapping'));
        $this->_initLayout()
            ->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_abandoned_opportunitylineitem'));
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_abandoned_opportunitylineitem_edit'));
            Mage::helper('tnw_salesforce')->addAdminhtmlVersion('TNW_Salesforce');
            $this->renderLayout();
        } else {
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            $data['sf_object'] = TNW_Salesforce_Model_Config_Objects::ABANDONED_ITEM_OBJECT;
    ...
    {
        $itemIds = $this->getRequest()->getParam('opportunitylineitem');
        if (!is_array($itemIds)) {
