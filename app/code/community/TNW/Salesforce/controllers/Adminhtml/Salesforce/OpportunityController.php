...
class TNW_Salesforce_Adminhtml_Salesforce_OpportunityController extends Mage_Adminhtml_Controller_Action
{
    protected function _initLayout()
    {
        ...
        $this->loadLayout()
            ...
            ->_addBreadcrumb(Mage::helper('tnw_salesforce')->__('Opportunity Fields Mapping'), Mage::helper('tnw_salesforce')->__('Opportunity Field Mapping'));
    ...
    {
        $this->_title($this->__('System'))->_title($this->__('Salesforce API'))->_title($this->__('Opportunity Field Mapping'));
        $this->_initLayout()
            ->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_opportunity'));
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $id == 0) {
            ...
            Mage::register('salesforce_opportunity_data', $model);
            ...
            $this->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_opportunity_edit'));
            Mage::helper('tnw_salesforce')->addAdminhtmlVersion('TNW_Salesforce');
            $this->renderLayout();
        } else {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if ($data['local_field'] == "Custom : field") {
                ...
                if ($mageGroup[0] == "Customer") {
                    ...
                    $data['attribute_id'] = $attr->getId();
                    $data['backend_type'] = $attr->getBackendType();
                } else if ($mageGroup[0] == "Product") {
                    $attrId = Mage::getResourceModel('eav/entity_attribute')
                        ->getIdByCode("catalog_product", $mageGroup[1]);
                    $attr = Mage::getModel('catalog/resource_eav_attribute')->load($attrId);
                    $data['attribute_id'] = $attr->getId();
                    $data['backend_type'] = $attr->getBackendType();
                } else if ($mageGroup[0] == "Order") {
                    $attrId = Mage::getResourceModel('eav/entity_attribute')
                        ->getIdByCode("order", $mageGroup[1]);
                    $attr = Mage::getModel('catalog/resource_eav_attribute')->load($attrId);
                    $data['attribute_id'] = $attr->getId();
                    $data['backend_type'] = $attr->getBackendType();
                }
    ...
    {
        $itemIds = $this->getRequest()->getParam('opportunity');
        if (!is_array($itemIds)) {
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('tnw_salesforce/opportunity');
    }
