...
class TNW_Salesforce_Adminhtml_Salesforce_AccountController extends Mage_Adminhtml_Controller_Action
{
    /**
     * @return $this
     */
    ...
    {
        $this->_title($this->__('System'))->_title($this->__('Salesforce API'))->_title($this->__('Account Field Mapping'));
        $this->_initLayout()
            ->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_account'));
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $id == 0) {
            ...
            Mage::register('salesforce_account_data', $model);
            ...
            $this->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_account_edit'));
            Mage::helper('tnw_salesforce')->addAdminhtmlVersion('TNW_Salesforce');
            $this->renderLayout();
        } else {
    ...
    {
        $itemIds = $this->getRequest()->getParam('account');
        if (!is_array($itemIds)) {
