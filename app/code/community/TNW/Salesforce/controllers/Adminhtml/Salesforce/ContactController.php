...
/**
 * Class TNW_Salesforce_Adminhtml_Salesforce_ContactController
 */
class TNW_Salesforce_Adminhtml_Salesforce_ContactController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->_title($this->__('System'))->_title($this->__('Salesforce API'))->_title($this->__('Contact Field Mapping'));
        $this->_initLayout()
            ->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_contact'));
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $id == 0) {
            ...
            Mage::register('salesforce_contact_data', $model);
            ...
            $this->_addContent($this->getLayout()->createBlock('tnw_salesforce/adminhtml_contact_edit'));
            //$this->_addLeft($this->getLayout()->createBlock('tnw_salesforce/adminhtml_contact_edit_tabs'));
            Mage::helper('tnw_salesforce')->addAdminhtmlVersion('TNW_Salesforce');
            $this->renderLayout();
        } else {
    ...
    {
        $itemIds = $this->getRequest()->getParam('contact');
        if (!is_array($itemIds)) {
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('tnw_salesforce/contact');
    }
