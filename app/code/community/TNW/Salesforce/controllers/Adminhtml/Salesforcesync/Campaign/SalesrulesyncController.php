...
class TNW_Salesforce_Adminhtml_Salesforcesync_Campaign_SalesrulesyncController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function syncAction()
    {
        ...
        if (!$_syncType) {
            Mage::getSingleton('adminhtml/session')
                ->addError("Integration Type is not set.");

            $this->_redirect("adminhtml/system_config/edit", array('section' => 'salesforce_order'));
            return;
        }

        $salesruleId = $this->getRequest()->getParam('catalogrule_id');
        if ($salesruleId > 0) {
            try {
                if (Mage::helper('tnw_salesforce')->getObjectSyncType() != 'sync_type_realtime') {
                    $res = Mage::getModel('tnw_salesforce/localstorage')
                        ->addObject(array($salesruleId), 'Campaign', 'salesrule');
    ...
    public function massSyncForceAction()
    {
        ...
        if (!is_array($itemIds)) {
            Mage::getSingleton('adminhtml/session')
                ->addError(Mage::helper('tnw_salesforce')->__('Please select sales rule(s)'));
        }
        ...
        elseif(((Mage::helper('tnw_salesforce')->getObjectSyncType() == 'sync_type_realtime')) && (count($itemIds) > 50)) {
            ...
            try {
                if (Mage::helper('tnw_salesforce')->getObjectSyncType() != 'sync_type_realtime') {
                    ...
                    $res = Mage::getModel('tnw_salesforce/localstorage')
                        ->addObject($itemIds, 'Campaign', 'salesrule');
