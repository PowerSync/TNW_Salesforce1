    ...
    protected function _getCustomer($quote)
    {
        ...
        if ($customer_id) {
            ...
            if (!$_customer->getId()) {
                ...
                Mage::helper('tnw_salesforce')->getDbConnection()->query($sql);
                Mage::getSingleton('tnw_salesforce/tool_log')->saveTrace('Guest user found in Magento, updating abandoned cart #' . $quote->getId() . ' attaching cusomter ID: ' . $_customer->getId());
            }
        }
        if (
