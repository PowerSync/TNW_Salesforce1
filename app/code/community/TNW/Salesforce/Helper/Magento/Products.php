...
class TNW_Salesforce_Helper_Magento_Products extends TNW_Salesforce_Helper_Magento_Abstract
{
        ...
        try {
            ...
            foreach ($this->_mapProductCollection as $_mapping) {
                if (strpos($_mapping->getLocalField(), 'Product : ') === 0) {
                    ...
                    if (property_exists($object, $_mapping->getSfField())) {
                        ...
                        if ($attOb->getFrontendInput() == 'select') {
                            ...
                            if (empty($_value)) {
                                ...
                                Mage::getModel('tnw_salesforce/tool_log')->saveNotice("SKIPPING: product code $sfValue not found in magento");
                                continue;
                            }
                    ...
                    if ($_value) {
                        Mage::getModel('tnw_salesforce/tool_log')->saveTrace('Product: ' . $_magentoFieldName . ' = ' . $_value);
                        $_product->setData($_magentoFieldName, $_value);
                    } else {
                        Mage::getModel('tnw_salesforce/tool_log')->saveTrace('SKIPPING Product: ' . $_magentoFieldName . ' - no value specified in Salesforce');
                    }
                    ...
                    if (property_exists($object, $_mapping->getSfField())) {
                        ...
                        Mage::getModel('tnw_salesforce/tool_log')->saveTrace('Product Inventory: ' . $_magentoFieldName . ' = ' . $object->{$_mapping->getSfField()});
                    }
                }
            }
            ...
            if (!$_product->getData('updated_at') || $_product->getData('updated_at') == '') {
                Mage::getModel('tnw_salesforce/tool_log')->saveTrace('Product: updated_at = ' . $_currentTime);
            ...
            if (!$_product->getData('created_at') || $_product->getData('created_at') == '') {
                ...
                Mage::getModel('tnw_salesforce/tool_log')->saveTrace('Product: created_at = ' . $_currentTime);
            ...
            Mage::getModel('tnw_salesforce/tool_log')->saveError("ERROR upserting product into Magento: " . $e->getMessage());
            unset($e);
            return false;
        }
    }
    ...
    public function syncFromSalesforce($object = null)
    {
        ...
        if (!$_salesforceId) {
            Mage::getModel('tnw_salesforce/tool_log')->saveError("ERROR upserting product into Magento: Product2 ID is missing");
            $this->_addError('Could not upsert Product into Magento, salesforce ID is missing', 'SALESFORCE_ID_IS_MISSING');
            return false;
        }
        if (!$_sku && !$_magentoId) {
            Mage::getModel('tnw_salesforce/tool_log')->saveError("ERROR upserting product into Magento: Email and Magento ID are missing");
        ...
        if ($_magentoId && !$_isNew) {
            Mage::getModel('tnw_salesforce/tool_log')->saveTrace("Product loaded using Magento ID: " . $_magentoId);
        } else {
            // No Magento ID
            if ($_salesforceId) {
                // Try to find the user by SF Id
            ...
            if ($_magentoId) {
                Mage::getModel('tnw_salesforce/tool_log')->saveTrace("Customer #" . $_magentoId . " Loaded by using Salesforce ID: " . $_salesforceId);
            } else {
                ...
                if ($_magentoId) {
                    Mage::getModel('tnw_salesforce/tool_log')->saveTrace("Customer #" . $_magentoId . " Loaded by using SKU: " . $_sku);
                } else {
                    //Brand new user
                    $_isNew = true;
                }
