...
class TNW_Salesforce_Helper_Order_Notes extends TNW_Salesforce_Helper_Order
{
    ...
    public function process($comment = NULL, $order = NULL, $type = NULL)
    {
        ...
        if (!$comment) {
            Mage::getSingleton('tnw_salesforce/tool_log')->saveTrace("Cannot update Note - Note object is missing");
            return false;
        }
        if (!$order) {
            Mage::getSingleton('tnw_salesforce/tool_log')->saveTrace("Cannot update Note - Order object is missing");
            return false;
        }
        Mage::getSingleton('tnw_salesforce/tool_log')->saveTrace("------------------- Notes Start -------------------");
        ...
        if (!$response[0]->success) {
            Mage::getSingleton('tnw_salesforce/tool_log')->saveTrace("Failed to upsert Note!");
            ...
            if ($_updateComment) {
                Mage::getSingleton('tnw_salesforce/tool_log')->saveTrace("Updating table: " . $_tableUpdate);
                try {
                    ...
                    Mage::getSingleton('tnw_salesforce/tool_log')->saveError("Failed to update table '" . $_tableUpdate . "': " . $e->getMessage());
                    unset($e);
                }
            }
            Mage::getSingleton('tnw_salesforce/tool_log')->saveTrace("Note #" . $response[0]->id . " upserted...");
        }
        unset($response);
        Mage::getSingleton('tnw_salesforce/tool_log')->saveTrace("------------------- Note End -------------------");
    }
}