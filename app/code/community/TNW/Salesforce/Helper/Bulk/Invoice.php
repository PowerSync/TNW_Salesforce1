...
class TNW_Salesforce_Helper_Bulk_Invoice extends TNW_Salesforce_Helper_Salesforce_Invoice
{
    protected function _pushEntity()
    {
        ...
        if (!$this->_cache['bulkJobs'][$this->_magentoEntityName]['Id']) {
            ...
            $this->_cache['bulkJobs'][$this->_magentoEntityName]['Id']
                = $this->_createJob(TNW_Salesforce_Model_Config_Objects::ORDER_INVOICE_OBJECT, 'upsert', 'Id');
    ...
    protected function _pushRemainingEntityData()
    {
        ...
        if (!empty($this->_cache[$itemToUpsertKey])) {
            if (!$this->_cache['bulkJobs'][$itemKey]['Id']) {
                ...
                $this->_cache['bulkJobs'][$itemKey]['Id']
                    = $this->_createJob(TNW_Salesforce_Model_Config_Objects::ORDER_INVOICE_ITEM_OBJECT, 'upsert', 'Id');
        ...
        // Mark invoice as failed or successful
        $this->_updateRecords();
    }
                ...
                foreach ($response as $_item) {
                    ...
                    if ($_item->success == "false") {
                        ...
                        if ($_recordItemId && strrpos($_recordItemId, 'cart_', -strlen($_recordItemId)) !== FALSE) {
                            $_sql = "UPDATE `" . Mage::helper('tnw_salesforce')->getTable('sales_flat_invoice_item') . "` SET salesforce_id = '" . $_item->id . "' WHERE entity_id = '" . str_replace('cart_','',$_recordItemId) . "';";
