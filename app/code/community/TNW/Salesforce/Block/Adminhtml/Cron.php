...
class TNW_Salesforce_Block_Adminhtml_Cron extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        $_lastTimestamp = 0;
        if ($_useCache) {
            $_lastTimestamp = (unserialize($_mageCache->load('tnw_salesforce_cron_timestamp'))) ? unserialize($_mageCache->load('tnw_salesforce_cron_timestamp')) : 0;
        }

        return date("l jS \of F Y h:i:s A", $_lastTimestamp);
    }
}