...
class TNW_Salesforce_Model_License
{
    ...
    protected function _testLicense()
    {
        ...
        if (!$this->_status) {
            Mage::helper('tnw_salesforce')->log("INFO: Invalid license!");
        }
    }
    ...
    protected function _isValid()
    {
        ...
        foreach ($this->_licenseServerUrlList as $serverUrl => $timestamp) {
            $serverPosition++;
            Mage::helper('tnw_salesforce')->log("License validation: server " . $this->_licenseServerUrlListAlias[$serverUrl] . " iteration started");
            ...
            if (intval($timestamp) > 0 && time() - $timestamp < self::FREEZE_DURATION) {
                ...
                Mage::helper('tnw_salesforce')->log("License validation: server " . $this->_licenseServerUrlListAlias[$serverUrl] . " skipped because it's frozen for the next $_freezePeriod seconds");
                continue;
            }
            ...
            try {
                ...
                if ($this->_status) {
                    Mage::helper('tnw_salesforce')->log("License validation: passed");
                    break;
                }
                ...
                if (!$this->_status && $serverPosition < count($this->_licenseServerUrlList)) {
                    Mage::helper('tnw_salesforce')->log("License validation issue: freezing server " . $this->_licenseServerUrlListAlias[$serverUrl]);
                    // update server freeze time
                    $this->_licenseServerUrlList[$serverUrl] = time();
                }
            } catch (Exception $e) {
                Mage::helper('tnw_salesforce')->log("License validation failed. Exception: " . $e->getMessage());
                // update server freeze time
                $this->_licenseServerUrlList[$serverUrl] = time();
            }
        }
