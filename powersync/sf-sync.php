...
Mage::getModel('tnw_salesforce/tool_log')->saveTrace("========== Sync from Salesforce Start ==========");
Mage::getModel('tnw_salesforce/tool_log')->saveTrace("JSON: ". urlencode($fromSf));
...
if (!$helper->isWorking()) {
    ...
    try {
        ...
        if (count($auth) != 2 || $auth[0] !== "BASIC" || !$myAuth || !is_array($credentials) || count($credentials) != 2) {
            Mage::getModel('tnw_salesforce/tool_log')->saveTrace('ERROR: Trying to hack, eh?');
        ...
        if ($helper->getLicenseInvoice() != $credentials[0] || $helper->getLicenseEmail() != $credentials[1]) {
            Mage::getModel('tnw_salesforce/tool_log')->saveTrace('ERROR: Authentication fialed!');
            throw new Exception("Authorization invalid, possible attack!");
        }
    } catch (Exception $e) {
        Mage::getModel('tnw_salesforce/tool_log')->saveTrace('ERROR: ' . $e->getMessage());
        exit;
    }
    ...
    if ($helper->getType() != "PRO") {
        ...
        if (!$objects || !is_array($objects)) {
            ...
            Mage::getModel('tnw_salesforce/tool_log')->saveTrace($errorString);
        } else {
            // Check if integration is enabled in Magento
            if (!$helper->isEnabled()) {
                Mage::getModel('tnw_salesforce/tool_log')->saveTrace('SKIPING: Synchronization is disabled');
            } else {
                if (count($objects) == 1) {
                    // Process Realtime
                    $object = $objects[0];
                    try {
                        ...
                        Mage::getModel('tnw_salesforce/tool_log')->saveTrace("Error: " . $e->getMessage());
                        Mage::getModel('tnw_salesforce/tool_log')->saveTrace("Failed to upsert a " . $object->attributes->type . " #" . $object->Id . ", please re-save or re-import it manually");
                    }
                } else {
                    // Add to Queue
                    /* Save into a db */
                    try {
                        Mage::getModel('tnw_salesforce/import')
                            ->setObject($fromSf)
                            ->save();
                        Mage::getModel('tnw_salesforce/tool_log')->saveTrace("Import JSON accepted, pending Import");
                    } catch (Exception $e) {
...
Mage::getModel('tnw_salesforce/tool_log')->saveTrace("Return JSON to Salesforce: " . $return);
Mage::getModel('tnw_salesforce/tool_log')->saveTrace("========== Sync from Salesforce End ==========");
